...
def go_post_order_traverse(tree):
    post_order_traverse(tree, 0)
    
def post_order_traverse(tree, node): # tree
    if not node in tree or len(tree[node]) == 0:
        print node
        return

    if tree[node][0] != -1:
        post_order_traverse(tree, tree[node][0])
    if tree[node][1] != -1:
        post_order_traverse(tree, tree[node][1])
    print node

    return
    
if "__main__" == __name__:
    go_post_order_traverse(tree1)

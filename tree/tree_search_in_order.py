...
def go_in_order_traverse(tree):
    in_order_traverse(tree, 0)
    
def in_order_traverse(tree, node): # tree
    if not node in tree or len(tree[node]) == 0:
        print node
        return

    if tree[node][0] != -1:
        in_order_traverse(tree, tree[node][0])
    print node
    if tree[node][1] != -1:
        in_order_traverse(tree, tree[node][1])

    return

if "__main__" == __name__:
    go_in_order_traverse(tree1)
